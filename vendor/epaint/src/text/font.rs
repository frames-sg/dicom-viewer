...
use crate::text::windows_directwrite::{DirectWriteFont, GlyphBitmap};
use crate::{
...
impl FontFace {
    pub fn new(
        ...
        #[cfg(target_os = "windows")]
        let directwrite = DirectWriteFont::new(std::sync::Arc::clone(&font_data), index);
    ...
    pub fn allocate_glyph(
        ...
        let allocation = self
            .directwrite
            .rasterize(glyph_id, metrics.scale, bin.as_float())
            .map(|bitmap| {
                glyph_allocation_from_directwrite(
                    atlas,
                    metrics,
                    glyph_id,
                    advance_width_px,
            ...
            .or_else(|| {
                self.font.allocate_glyph_uncached(
                    atlas,
                    metrics,
                    &glyph_info,
                    bin,
...
#[cfg(target_os = "windows")]
fn glyph_allocation_from_directwrite(
    atlas: &mut TextureAtlas,
    metrics: &StyledMetrics,
    glyph_id: skrifa::GlyphId,
        ...
        for y in 0..height {
            for x in 0..width {
                image[(x + glyph_pos.0, y + glyph_pos.1)] =
                    alpha_from_coverage.color_from_coverage(coverage[y * width + x] as f32 / 255.0);
            }
        }
        UvRect {
            offset: vec2(left as f32, top as f32) / metrics.pixels_per_point
